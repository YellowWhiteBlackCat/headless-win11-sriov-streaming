...
if [[ -f secrets/secrets.local.env ]]; then
    set -a
    # shellcheck disable=SC1091
    source secrets/secrets.local.env
    set +a
fi

ADMIN_PASSWORD="${ADMIN_PASSWORD:-}"
SSH_PUB_KEY="${SSH_PUB_KEY:-secrets/admin_ed25519.pub}"

if [[ ! -f "$SSH_PUB_KEY" ]]; then
    echo "ERROR: SSH public key not found at $SSH_PUB_KEY" >&2
    echo "Generate one with: ssh-keygen -t ed25519 -f secrets/admin_ed25519 -N ''" >&2
    exit 1
fi

if [[ -z "$ADMIN_PASSWORD" ]]; then
    echo "ERROR: ADMIN_PASSWORD is not set." >&2
    echo "Export it or create secrets/secrets.local.env from secrets.local.env.example." >&2
    exit 1
fi
